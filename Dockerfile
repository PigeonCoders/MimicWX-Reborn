# MimicWX-Reborn Docker 环境
# 构建: docker build -t mimicwx-reborn .
...
# ================================================================
# Stage 1: Planner (分析依赖图谱)
# ================================================================
FROM ubuntu:22.04 AS planner
ARG USE_MIRROR
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    libxcb1-dev libxrandr-dev libxext-dev libssl-dev && \
...
# 安装 cargo-chef (用于分析依赖)
RUN cargo install cargo-chef

WORKDIR /build
# 复制所有 Cargo.toml 和源码以分析依赖
COPY Cargo.toml Cargo.lock ./
COPY crates/ crates/
RUN cargo chef prepare --recipe-path recipe.json

# ================================================================
# Stage 2: Builder (编译依赖 + 编译项目)
# ================================================================
...
# 1. 构建依赖缓存 (这一步会被 Docker 缓存，只有依赖变了才会重新跑)
