  ...
  new (class MimicWXAdapter {
    ...
    async initBot() {
      ...
      Bot[this.self_id] = {
        ...
        getGroupMap: () => gl,
        getGroupMemberMap: () => Bot[this.self_id]?.gml || new Map(),
      ...
      // 异步加载群成员列表 (不阻塞连接初始化)
      this.loadGroupMembers(gl)
    }

    /**
     * 异步加载所有群的成员列表到 bot.gml
     */
    async loadGroupMembers(gl) {
      const bot = Bot[this.self_id]
      if (!bot) return
      let total = 0
      for (const [group_id] of gl) {
        try {
          const res = await fetch(`${MIMICWX_URL}/group_members?group=${encodeURIComponent(group_id)}`, { headers: authHeaders() })
          if (!res.ok) continue
          const data = await res.json()
          const memberMap = new Map()
          for (const m of data.members || []) {
            memberMap.set(m.wxid, {
              user_id: m.wxid,
              nickname: m.nick_name || m.wxid,
              card: m.display_name || m.nick_name || m.wxid,
              group_id,
              bot_id: this.self_id,
            })
          }
          bot.gml.set(group_id, memberMap)
          total += memberMap.size
        } catch (e) {
          Bot.makeLog("debug", `群成员加载失败 ${group_id}: ${e.message}`, this.self_id)
        }
      }
      if (total > 0) Bot.makeLog("info", `群成员加载完成: ${gl.size}群, ${total}人`, this.self_id)
    }
    ...
    pickGroup(group_id) {
      ...
      const gml = Bot[this.self_id]?.gml
      return {
        ...info,
        group_id,
        sendMsg: (msg) => {
        ...
        getInfo: () => info,
        getAvatarUrl: () => null,
        getMemberArray: () => [...(gml?.get(group_id)?.values() || [])],
        getMemberList: () => [...(gml?.get(group_id)?.keys() || [])],
        getMemberMap: () => gml?.get(group_id) || new Map(),
        pickMember: (uid) => {
          const m = gml?.get(group_id)?.get(uid)
          const nickname = m?.nickname || m?.card || uid
          return {
            ...m,
            user_id: uid,
            nickname,
            card: m?.card || nickname,
            sendMsg: (msg) => this.sendMsgSmart(nickname, msg),
            getInfo: () => m || { user_id: uid, nickname: uid },
            getAvatarUrl: () => null,
          }
        },
      }
