...
//! - POST /send_image    — 发送图片 (AT-SPI)
//! - POST /chat          — 切换聊天 (AT-SPI)
//! - POST /listen        — 添加监听 (弹出独立窗口)
//! - DELETE /listen      — 移除监听
//! - GET  /listen        — 监听列表
//! - GET  /group_members — 群成员列表
//! - POST /command       — 通用命令执行
//! - GET  /debug/tree    — AT-SPI2 控件树
//! - GET  /debug/sessions — 会话容器子树
...
use mimicwx_atspi::{AtSpi, dump_tree};
...
pub fn spawn_input_actor(
    ...
    tokio::spawn(async move {
        info!(target: "mimicwx::api", "InputEngine actor 已启动");
        ...
        info!(target: "mimicwx::api", "InputEngine actor 已停止");
    });
}
...
async fn auth_layer(
    ...
    warn!(target: "mimicwx::api", "认证失败: {}", req.uri().path());
...
pub fn build_router(state: Arc<AppState>) -> Router {
    let protected = Router::new()
        ...
        .route("/group_members", get(get_group_members))
...
async fn get_group_members(
    ...
    let db = state.db.as_ref().ok_or_else(|| ApiError::unavailable("数据库不可用"))?;
    let group = params.get("group").ok_or_else(|| ApiError::internal("缺少 group 参数"))?;
    match db.get_group_members(group).await {
        Ok(members) => Ok(Json(serde_json::json!({
            "group": group,
            "count": members.len(),
            "members": members,
        }))),
        Err(e) => Err(ApiError::internal(format!("群成员查询失败: {e}"))),
    }
}
...
const MAX_IMAGE_BASE64_LEN: usize = 27 * 1024 * 1024;
...
async fn send_message(
    ...
    if req.text.len() > MAX_TEXT_LEN {
        return Err(ApiError::internal(format!(
            "文本过长: {} 字节 (上限 {} 字节)", req.text.len(), MAX_TEXT_LEN
        )));
    }
    ...
    match reply_rx.await {
        Ok(Ok((sent, atspi_verified, message))) => {
            let (verified, message_id) = if let (Some(rx), Some(db)) = (sent_rx, state.db.as_ref()) {
                match db.verify_sent(&req.text, rx).await {
                    Some(local_id) => (true, Some(format!("mimicwx_{}", local_id))),
                    None => (atspi_verified, None),
                }
            } else {
                (atspi_verified, None)
            };
            ...
            Ok(Json(SendResponse { sent, verified, message, message_id }))
        }
...
async fn send_image(
    ...
    if req.file.len() > MAX_IMAGE_BASE64_LEN {
        return Err(ApiError::internal(format!(
            "图片过大: {:.1}MB (上限 20MB)",
            req.file.len() as f64 / 1024.0 / 1024.0 * 3.0 / 4.0
        )));
    }
    ...
    match result {
        Ok(Ok((sent, verified, message))) => Ok(Json(SendResponse { sent, verified, message, message_id: None })),
...
async fn get_sessions(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    if let Some(db) = &state.db {
        match db.get_sessions().await {
            ...
            Err(e) => {
                tracing::warn!(target: "mimicwx::session", "DB 会话查询失败, fallback AT-SPI: {e}");
            }
        }
    }
...
async fn get_tree(
    ...
    if let Some(app) = state.wechat.find_app().await {
        let tree = dump_tree(state.atspi.as_ref(), &app, max_depth).await;
        Json(tree)
    } else {
        Json(vec![])
    }
...
async fn get_session_tree(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    if let Some(app) = state.wechat.find_app().await {
        if let Some(container) = state.wechat.find_session_list(&app).await {
            let tree = dump_tree(state.atspi.as_ref(), &container, 4).await;
            return Json(tree);
        }
    }
    Json(vec![])
...
async fn handle_ws(mut socket: WebSocket, state: Arc<AppState>) {
    ...
    debug!(target: "mimicwx::api", "WebSocket 连接建立");
    ...
    debug!(target: "mimicwx::api", "WebSocket 连接断开");
}
...
async fn exec_command(
    ...
    info!(target: "mimicwx::api", "远程命令: {cmd}");
    ...
    info!(target: "mimicwx::api", "命令结果: {result}");
...
async fn exec_reload(state: &AppState) -> String {
    ...
    let new_config: AppConfig = match toml::from_str(&content) {
        Ok(c) => c,
    ...
    match reply_rx.await {
        Ok(Ok(true)) => {
            if let Some(ref path) = state.config_path {
                ...
                mimicwx_core::save_listen_list(path, &list);
            }
            format!("[ok] 监听已添加: {who}")
        }
